...
def calc_change(trial,mask):
    ...
    mask_t = smoo[-1] - smoo[0]
    mask_t = np.absolute(mask_t)
    mask_t[mask_t < 1] = 0
    mask_t[np.isnan(mask_t)] = 0
    mask = morphology.remove_small_objects(mask_t.astype(bool), 1000)

    np.save('./'+ trial+ '/'+ "hourlyMask_bower.npy", mask)
...
def plotter(hourlyChange,mask,length_trial,bins,count):
    ...
    for i in range(hourlyChange.shape[0]):
        ...
        if len(abs_data) != 0:
            bins.append([trial,0.2,1000,i,len(abs_data),-np.nansum(tdata),np.nansum(abs_data),x_values[-1][-1],x_values[-1.33333][-1],x_values[-1.666666][-1],x_values[-2][-1],x_values[-2.33333][-1],x_values[-2.6666666][-1],x_values[-3][-1]])
    ...
    for power in sorted(list(x_values.keys())):
        ...
        avg_change[power].append(np.mean(np.array(x_values[power])[np.array(x_values[power]) != 0])/np.mean(np.array(abs_values[power])))
        ...
        if len(np.array(x_values[power])[np.array(x_values[power]) > 0]) != 0 and len(np.array(x_values[power])[np.array(x_values[power]) != 0]) != 0:
            max_height[power].append(np.sum(np.array(x_values[power])[np.array(x_values[power]) > 0])/np.sum(np.array(abs_values[power])))
        else:
            max_height[power].append(0)
        if len(np.array(x_values[power])[np.array(x_values[power]) < 0]) != 0 and len(np.array(x_values[power])[np.array(x_values[power]) != 0]) != 0:
            min_depth[power].append(np.sum(np.array(x_values[power])[np.array(x_values[power]) < 0])/np.sum(np.array(abs_values[power])))
        else:
...
for trial in trials:
    ...
    try:
        mask = np.load("./"+trial + "/hourlyMask_bower.npy")
        ...
        mask = np.empty(shape = (smoo.shape[1], smoo.shape[2]), dtype = bool)
        hourlyChange = np.load("./"+trial + "/hourlyChange.npy")

        print("Creating the mask")
        calc_change(trial,mask)
        del smoo
    length_trial = int(hourlyChange.shape[0])
...
with open("all.csv",'w') as csvfile:
    fieldnames = ['trial','min_depth','min_region', 'frame_num', 'pixels_passing', 'actual_change', 'absolute_change',str((10**-1)*100),str((10**-1.33333)*100),str((10**-1.666666)*100),str((10**-2)*100),str((10**-2.33333)*100),str((10**-2.6666666)*100),str((10**-3)*100) ]
